...
/// Holds the built tools list plus a reverse map for names that were shortened.
/// Map: short_tool_name -> original path segments [svc, res1, ..., method]
struct ToolRegistry {
    tools: Vec<Value>,
...
fn make_tool_name(parts: &[String]) -> String {
    parts
        .iter()
        .map(|s| shorten_segment(s))
        .collect::<Vec<_>>()
        .join("_")
}
...
async fn build_tools_list(config: &ServerConfig) -> Result<ToolRegistry, GwsError> {
    ...
    for svc_name in &config.services {
        ...
        if let Ok(doc) = crate::discovery::fetch_discovery_document(&api_name, &version).await {
            walk_resources(&[doc.name.clone()], &doc.resources, &mut tools, &mut name_map);
        } else {
...
fn walk_resources(
    ...
    for (res_name, res) in resources {
        ...
        new_prefix.push(res_name.clone());

        for (method_name, method) in &res.methods {
            let mut parts = new_prefix.clone();
            parts.push(method_name.clone());
            let tool_name = make_tool_name(&parts);

            // Store reverse mapping only when segments were actually abbreviated
            let original_name = parts.join("_");
            if tool_name != original_name {
                name_map.insert(tool_name.clone(), parts);
            }
...
async fn handle_tools_call(params: &Value, config: &ServerConfig, name_map: &HashMap<String, Vec<String>>) -> Result<Value, GwsError> {
    ...
    // If the name was shortened, look up the original segments from the registry.
    let owned_parts: Vec<String>;
    let parts: Vec<&str> = if let Some(original_parts) = name_map.get(tool_name) {
        owned_parts = original_parts.clone();
        owned_parts.iter().map(|s| s.as_str()).collect()
    } else {
        owned_parts = tool_name.split('_').map(|s| s.to_string()).collect();
        owned_parts.iter().map(|s| s.as_str()).collect()
    };
    ...
    // Walk resource segments between service and method
